//
//  BatteryFrequencyViewController.swift
//  SmartLock
//
//  Created by Innoflexion on 19/11/22.
...
class BatteryFrequencyViewController: UIViewController, UITextFieldDelegate {

    @IBOutlet weak var batteryFrequencyTextField: UITextField!
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        handleDisconnect()
        batteryFrequencyTextField.delegate = self
    }
    ...
    @IBAction func didTapOnSubmitButton(_ sender: Any) {
        if let text = batteryFrequencyTextField.text, !text.isEmpty {
            let iotDataManager = AWSIoTDataManager(forKey: AWS_IOT_DATA_MANAGER_KEY)
            iotDataManager.publishString("\"BattCheckFreq\(text)\"", onTopic:getTopic(), qoS:.messageDeliveryAttemptedAtMostOnce)
            self.toastMessage("Successfully updated")
        } else {
            self.toastMessage("Please enter battery frequency")
        }
    }
