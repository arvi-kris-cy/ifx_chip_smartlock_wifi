...
- (void)viewDidLoad
{
    ...
    [_deviceList forSelectedDevices:^(uint64_t deviceId) {
        
        MTRUnpairDeviceWithID(deviceId);
    }];
    [_deviceList refreshDeviceList];
   // [self setupUI];
    [self updateUILayout];
    ...
    // Start
    [self toastMessage:@"Scanning for device" duration:2.0];
...
//        // pairing error - CHANDRA
//        // start
//        [self toastMessage:@"Connection unsuccessful" duration:3];
//        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 3 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
//            [self toastMessage:@"Check Wi-Fi credentials and mobile Wi-Fi connection" duration:2];
//            [self.navigationController popViewControllerAnimated:YES];
//        });
//        // end
//    } else {
...
//        }
//        // Start
//        [self toastMessage:@"Device found, pairing…" duration:8.0];
//        // End
//    }
//}
...
- (void)retrieveAndSendWiFiCredentials
{
    UIAlertController * alertController =
        [UIAlertController alertControllerWithTitle:@"Network pairing credentials"
                                            message:@"\nEnter your Wi-Fi network ID and password to connect to the network.\n\n Ensure mobile is connected to the same network."
...
- (void)onCommissioningComplete:(NSError * _Nullable)error
{
    ...
    if (error != nil) {
        ...
        // end
        NSLog(@"Error retrieving device informations over Mdns: %@", error);        return;
    }
    // track this device
    [self toastMessage:@"Connected successfully" duration:2.0];
    uint64_t deviceId = MTRGetNextAvailableDeviceID() - 1;
    MTRSetDevicePaired(deviceId, YES);
    [self setVendorIDOnAccessory];
    dispatch_async(dispatch_get_main_queue(), ^{
       // [self toastMessage:@"Connection successful" duration:3];
        UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
        SLOnOffViewController *myNewVC = (SLOnOffViewController *)[storyboard instantiateViewControllerWithIdentifier:@"SLOnOffViewController"];
        myNewVC.isFromHomeScreen = YES;
...
- (void)errorGoBackWithAlert
{
    [self toastMessage:@"An error occured while connecting with device" duration:2];
...
- (void)parseOptionalData:(MTRSetupPayload *)payload
{
    ...
    NSLog(@"Payload vendorID %@", payload.discriminator);
    NSLog(@"Payload vendorID %@", payload.productID);
    NSString* smartLockDesc = [NSString stringWithFormat:@"SmartLock_%@", payload.discriminator];
    NSString* toastMessage = [NSString stringWithFormat:@"Connecting to SmartLock_%@", payload.discriminator];
    [self toastMessage:toastMessage duration:2.0];
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    [defaults setObject:smartLockDesc forKey:@"discriminatorSaved"];
...
- (IBAction)submitButtonTapped:(id)sender {
    if ([_deviceAddressTF.text  isEqual: @""]) {
        UIAlertView * alert = [[UIAlertView alloc]initWithTitle:@"" message:@"Please enter device address" delegate:self cancelButtonTitle:@"ok" otherButtonTitles:nil];
        [alert show];
    } else {
        NSString * decimalString = _deviceAddressTF.text;
        [self manualCodeEnteredStartState];
        ...
        [_deviceAddressTF resignFirstResponder];
    }
}
