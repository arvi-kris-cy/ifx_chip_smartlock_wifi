//
//  PersonalizeCardViewController.swift
//  SmartLock
//
...
class PersonalizeCardViewController: UIViewController {
    ...
    @IBAction func createNewCardButton(_ sender: Any) {
        let iotDataManager = AWSIoTDataManager(forKey: AWS_IOT_DATA_MANAGER_KEY)
        iotDataManager.publishString("\"AddNFC\"", onTopic:getTopic(), qoS:.messageDeliveryAttemptedAtMostOnce)
        self.toastMessage("Create status published to topic \(getTopic())")
    }
    
    
    @IBAction func provisionNewCardButton(_ sender: Any) {
        let iotDataManager = AWSIoTDataManager(forKey: AWS_IOT_DATA_MANAGER_KEY)
        iotDataManager.publishString("\"Prov_NFC\"", onTopic:getTopic(), qoS:.messageDeliveryAttemptedAtMostOnce)
        self.toastMessage("Provision status published to topic \(getTopic())")
    }
    
     @IBAction func blockAllCardsButton(_ sender: Any) {
         let iotDataManager = AWSIoTDataManager(forKey: AWS_IOT_DATA_MANAGER_KEY)
         iotDataManager.publishString("\"Deprov_NFC\"", onTopic:getTopic(), qoS:.messageDeliveryAttemptedAtMostOnce)
         self.toastMessage("Blocked all cards")
     }
